...
########1. Variant filteration
	...
	$gatk --java-options -Xmx20g SelectVariants --exclude-filtered  -V $sample/$sample.BQSR.SNP.filter.vcf -O $sample/$sample.BQSR.SNP.filtered.vcf

	$gatk --java-options -Xmx20g SelectVariants --exclude-filtered  -V $sample/$sample.BQSR.Indel.filter.vcf -O $sample/$sample.BQSR.Indel.filtered.vcf

	$gatk --java-options -Xmx20g MergeVcfs -I $sample/$sample.BQSR.SNP.filtered.vcf -I $sample/$sample.BQSR.Indel.filtered.vcf -O $sample/$sample.merge.BQSR.filtered.vcf
...
########3. variant annotation with ANNOVAR v.2016-02-01
	...
	########2.2. annotate the VCF files with Annovar
